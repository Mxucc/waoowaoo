...
type ManualAssetKind = 'image' | 'video' | 'audio' | 'text'
...
function parseManualAsset(payload: unknown): ManualAsset | null {
  ...
  if (kind !== 'image' && kind !== 'video' && kind !== 'audio' && kind !== 'text') return null
...
export default function WorkspaceManualAssetModal() {
  ...
  const onUpload = async (item: ManualAssetItem) => {
    ...
    const isTextMode = manualAsset.kind === 'text'
    const file = isTextMode ? null : (fileByKey[item.key] || null)
    const text = isTextMode ? (textByKey[item.key] || '') : ''
    if (!isTextMode && !file) {
      setError('请选择文件')
      return
    }
    ...
    try {
      ...
      const res = await (async () => {
        ...
        formData.append(fileField, file as File)
      ...
      if (!res.ok) {
        const msg = asString(asObject(json)?.error)
          || asString(asObject(json)?.message)
          || '上传失败'
        throw new Error(msg)
      }
      ...
      setTextByKey((prev) => ({ ...prev, [item.key]: '' }))
                  ...
                  />
                    ...
                    {kind === 'text' ? (
                      <textarea
                        ...
                        onChange={(e) => setTextByKey((prev) => ({ ...prev, [item.key]: e.target.value }))}
                      />
                    ) : (
                      <input
                        type="file"
