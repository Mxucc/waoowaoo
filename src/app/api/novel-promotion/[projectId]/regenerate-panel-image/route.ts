...
export const POST = apiHandler(async (
  ...
  const storyboardModelKey = projectModelConfig.storyboardModel || null
  ...
  if (!storyboardModelKey) {
    throw new ApiError('INVALID_PARAMS', {
      code: 'STORYBOARD_MODEL_NOT_CONFIGURED',
    })
  }
  try {
    await resolveModelSelection(session.user.id, storyboardModelKey, 'image')
  } catch (error) {
